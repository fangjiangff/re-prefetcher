...
SRC = HERE / "test-same-pc-different-memory.c"
BIN_DIR = HERE / "bin"
BIN = BIN_DIR / "test-same-pc-different-memory"
RES_DIR = HERE / "res"
...
def pretty_section_name(name):
    names = {
        ...
        "same_pc_different_memory": "Same PC, different memory",
    }
...
def output_path_for_mode(path_arg, arch, mode, timestamp, suffix):
    if path_arg is None:
        return RES_DIR / f"{arch}-same-pc-different-memory-{mode}-{timestamp}.{suffix}"
...
def combined_plot_path(path_arg, arch, timestamp):
    if path_arg is None:
        return RES_DIR / f"{arch}-same-pc-different-memory-combined-{timestamp}.png"
...
def run_one_mode(args, mode, timestamp, save_individual_plot):
    ...
    if save_individual_plot:
        plotted = plot_result(result.stdout, plot_path, f"test-same-pc-different-memory [{args.arch}] ({mode})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Build and run the SMS same-PC different-memory test."
    )
    ...
    if len(outputs) > 1:
        ...
        plot_path.parent.mkdir(parents=True, exist_ok=True)
        plotted = plot_combined(outputs, plot_path, f"test-same-pc-different-memory [{args.arch}]")
