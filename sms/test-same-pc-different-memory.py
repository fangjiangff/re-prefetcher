...
def output_path_for_mode(path_arg, mode, timestamp, suffix):
    if path_arg is None:
        return RES_DIR / f"same-pc-different-memory-{mode}-{timestamp}.{suffix}"
...
def combined_plot_path(path_arg, timestamp):
    if path_arg is None:
        return RES_DIR / f"same-pc-different-memory-combined-{timestamp}.png"
...
def run_one_mode(args, mode, timestamp, save_individual_plot):
    ...
    if save_individual_plot:
        plotted = plot_result(result.stdout, plot_path, f"test-same-pc-different-memory ({mode})")
...
def main():
    ...
    if len(outputs) > 1:
        ...
        plot_path.parent.mkdir(parents=True, exist_ok=True)
        plotted = plot_combined(outputs, plot_path, "test-same-pc-different-memory")
