...
/*
 * Keep training and trigger loads at the same instruction PC.
 */
__attribute__((noinline)) static void same_pc_load(void *addr) {
    asm volatile("ldrb w0, [%0]\n\t" :: "r"(addr) : "memory", "w0");
}
 ...
 */
__attribute__((noinline)) static void same_sw_load(void *addr) {
    asm volatile("prfm pldl1keep, [%0]\n\t" :: "r"(addr) : "memory");
}
...
static void run_test(const char *name, int touch_extra_regions,
    ...
    for (int round = 0; round < rounds; round++) {
        ...
        for (size_t i = 0; i < sizeof(training_lines) / sizeof(training_lines[0]); i++) {
            train_or_trigger_access(training_region + training_lines[i] * CACHE_LINE_SIZE);
        }
        mfence();
...
int main(int argc, char **argv) {
    ...
    printf("# SMS same-PC different-memory test\n");
    printf("# access mode: %s\n",
           use_sw_prefetch ? "software prefetch (prfm pldl1keep)" : "load (ldrb)");
    printf("# training lines in training_region: 4, 1, 6, 7, 3, 11, 15\n");
    printf("# trigger line in test_region: 4\n");
