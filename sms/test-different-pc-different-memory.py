...
def output_path_for_mode(path_arg, arch, mode, timestamp, suffix):
    if path_arg is None:
        return RES_DIR / f"{arch}-different_pc_different_memory-{mode}-{timestamp}.{suffix}"
...
def combined_plot_path(path_arg, arch, timestamp):
    if path_arg is None:
        return RES_DIR / f"{arch}-different_pc_different_memory-combined-{timestamp}.png"
...
def run_one_mode(args, mode, timestamp, save_individual_plot):
    ...
    if save_individual_plot:
        plotted = plot_result(result.stdout, plot_path, f"test-different_pc_different_memory [{args.arch}] ({pretty_mode_name(mode)})")
...
def main():
    ...
    if len(outputs) > 1:
        ...
        plot_path.parent.mkdir(parents=True, exist_ok=True)
        plotted = plot_combined(outputs, plot_path, f"test-different_pc_different_memory [{args.arch}] (load vs software prefetch)")
