...
/*
 * PC B: trigger instruction. Keep this separate from train_load().
 */
...
static void run_test(const char *name, int touch_extra_regions,
    ...
    for (int round = 0; round < rounds; round++) {
        ...
        for (size_t i = 0; i < sizeof(training_lines) / sizeof(training_lines[0]); i++) {
            train_access(test_region + training_lines[i] * CACHE_LINE_SIZE);
        }
        mfence();
...
int main(int argc, char **argv) {
    ...
    printf("# SMS different-PC same-memory test\n");
    printf("# access mode: %s\n",
           use_sw_prefetch ? "software prefetch (prfm pldl1keep)" : "load (ldrb)");
    printf("# training lines in test_region via PC A: 4, 1, 6, 7, 3, 11, 15\n");
