...
SRC = HERE / "test-entries.c"
BIN_DIR = HERE / "bin"
BIN = BIN_DIR / "test-entries"
RES_DIR = HERE / "res"
...
def summarize_output(mode, entries, output, prefetch_min):
    ...
    return {
        ...
        "raw_entries": int(metadata.get("entries", entries)),
    }
...
def save_summary(path, summaries):
    ...
    with path.open("w") as f:
        f.write("mode\tentries\tdetected_prefetch_lines\texpected_lines\tavg_expected_per_1000\ttrigger_per_1000\n")
        for row in summaries:
            f.write(
                f"{row['mode']}\t{row['entries']}\t{row['detected_prefetch_lines']}\t"
                f"{row['expected_lines']}\t{row['avg_expected_per_1000']:.2f}\t"
                f"{row['trigger_per_1000']}\n"
            )
...
def plot_summary(path, summaries, title):
    ...
    for mode, rows in by_mode.items():
        rows = sorted(rows, key=lambda row: row["entries"])
        x = [row["entries"] for row in rows]
    ...
    axes[1].set_xlabel("Inserted training entries")
    axes[1].set_ylim(-0.2, 4.2)
...
def main():
    parser = argparse.ArgumentParser(
        description="Build and run the SMS training-entry capacity test."
    )
    ...
    if args.min_entries < 0 or args.max_entries < args.min_entries:
        raise SystemExit("invalid entry range")
    ...
    prefix = args.output_prefix or f"{args.arch}-entries-{timestamp}"
    ...
    for mode in modes:
        ...
        with raw_path.open("w") as raw:
            for entries in range(args.min_entries, args.max_entries + 1):
                cmd = [
                    "taskset",
                    "-c",
                    str(args.core),
                ...
                summaries.append(summarize_output(mode, entries, result.stdout, args.prefetch_min))
    ...
    plot_summary(plot_path, summaries, f"test-entries [{args.arch}] (load vs software prefetch)")
