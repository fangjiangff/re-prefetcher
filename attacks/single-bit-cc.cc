#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <sched.h>
#include "cacheutils.hh"
#include "utils.hh"
#include <random>
#include <sys/mman.h>
#include <vector>
// #include <x86intrin.h>
#include "time.h"


#define CPU_ID 0
#define LINE_SIZE 64
// #define Items 256
#define Items 2048
#define Prefetch_Threshold 200



#ifndef TEST_ON_SW
#define TEST_ON_SW 0
#endif

#ifndef TEST_ON_ST
#define TEST_ON_ST 0
#endif

#if TEST_ON_ST == 1
    /*
     * Store: 向 addr 写 1 字节
     * 使用 w 寄存器（低 8 bit 会被使用）
     */
    #define _maccess(pre, addr) \
        asm volatile( \
            pre "strb w0, [%0]\n\t" \
            :: "r" (addr) \
            : "memory", "w0")

#else
    /*
     * Load: 从 addr 读 1 字节
     */
    #define _maccess(pre, addr) \
        asm volatile( \
            pre "ldrb w0, [%0]\n\t" \
            :: "r" (addr) \
            : "memory", "w0")

#endif

uint8_t array1[100*64]={0};

#define REG_ARG_1 "x0"

#define mfence() asm volatile("DMB SY\nISB")

#define flush(addr) asm volatile("DC CIVAC, %0" :: "r" (addr))

#define return_asm() "ret"

#define nop() asm volatile("nop")

#define VIRTUAL_ADDRESS_BITS 48

void maccess(void *p) {
    _maccess("", p);
    // volatile uint32_t value;
    // asm volatile("LDR %0, [%1]\n\t" : "=r"(value) : "r"(p));
}

// static inline __attribute__((always_inline)) 
void mprefetch(void *p) {
    asm volatile(
        "PRFM PLDL1KEEP, [%0]\n\t"
        :
        : "r"(p)
        : "memory"
    );
}

static inline __attribute__((always_inline))
uint64_t timestamp1(void)
{
    asm volatile("DSB SY");
    asm volatile("ISB");
    struct timespec t1;
    clock_gettime(CLOCK_MONOTONIC, &t1);
    uint64_t res = t1.tv_sec * 1000 * 1000 * 1000ULL + t1.tv_nsec;
    asm volatile("ISB");
    asm volatile("DSB SY");
    return res;
}

static inline __attribute__((always_inline)) uint64_t timestamp() {
    uint64_t value;
    asm volatile("mrs %0, PMCCNTR_EL0" : "=r" (value));
    return value;
}
static inline __attribute__((always_inline)) uint64_t victim_probe(void *p) {
    register uint64_t start, end;
    mfence();
    start = timestamp();
    mfence();
    maccess(p);
    mfence();
    end = timestamp();
    mfence();  
    return end - start;
}



uint8_t array2[Items * LINE_SIZE] __attribute__((aligned(4096)));;
// uint8_t *array2;

long long int res2[100][100] = {0};

#define DUMMY_BUFFER_SIZE (PAGE_SIZE * 10)
#define DUMMY_LINES (DUMMY_BUFFER_SIZE / LINE_SIZE)

static uint8_t* dummy_buffer;

void dummyAccesses(){
        // printf("i am here.");
     for(uint64_t i = 0; i < DUMMY_LINES; i ++){
        if(TEST_ON_SW){
            asm volatile("PRFM PLDL3KEEP, [%0]\n\t" :: "r"(&dummy_buffer[i*64]));
        }else{
            asm volatile("LDR w0, [%0]\n\t" :: "r"(&dummy_buffer[i*64]) : "memory", "w0");
        }
        // mfence();
         // asm volatile("PRFM PLDL3STRM, [%0]\n\t" :: "r"(&dummy_buffer[i*64]));
     }
}

int main(int argc, char **argv){
  register uint64_t time1, time2;
  volatile uint8_t * probe_addr;
  unsigned int junk = 0;

  int rounds = 1000;
  if(argc > 1){
    char *end = NULL;
    long parsed_rounds = strtol(argv[1], &end, 10);
    if(*end != '\0' || parsed_rounds <= 0){
      fprintf(stderr, "Usage: %s [positive_attack_rounds]\n", argv[0]);
      return 1;
    }
    rounds = (int)parsed_rounds;
  }

  struct timespec const t_req{ .tv_sec = 0, .tv_nsec = 1500 /* 1µs */ };//秒0，纳秒1us
  struct timespec t_rem;


  memset(array2,-1,Items*LINE_SIZE*sizeof(uint8_t));
  dummy_buffer = (uint8_t*)mmap(NULL, DUMMY_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, 0, 0);
  if(dummy_buffer == MAP_FAILED) {
      printf("failed to map memory to access!\n");
      exit(1);
  }

    int i;    
    for(int i=0; i< Items; i++){
        maccess(&array2[i * 64]);
    }
    for (uint64_t offset = 0; offset < Items*LINE_SIZE; offset+=LINE_SIZE){
        flush(&array2[offset]);
    }

  std::vector<int> secret(rounds);
  for(int i=0;i<rounds;i++){
    secret[i] = rand() % 2;// 0 or 1
    // secret[i] = 1;// 0 or 1
    // printf("secret %d: %d\n", i, secret[i]);
  }

  // for(int stride = 64*1; stride <= 64*64; stride+=64){
    //   printf("Stride %d*64:\t%d\t\t",stride/64,stride);
    int stride1 = 17 * 64;
    int stride2 = 30 * 64;
    int stride = stride1;
    
    int train_step = 20;//>16 wil be more stable, but 16 is already good enough.
    // for(int train_step = 1; train_step <= 20 ; train_step++){

      printf("Stride %d*64:\t%d\ttrian_step %d\t",stride/64,stride, train_step);
      uint64_t latency = 0;
      std::vector<int> res1(rounds);
      std::vector<int> res2(rounds);
      uint64_t attack_start = timestamp1();
          for(uint64_t atkRound = 0; atkRound < rounds; ++atkRound) {
            if(secret[atkRound]==1){
                stride = stride1;
            }else{
                stride = stride2;
            }
               for (uint64_t offset = 0; offset < Items*LINE_SIZE; offset+=LINE_SIZE){
                    flush(&array2[offset]);
               }
               dummyAccesses();//for dummy accesses , reset the prefetcher state
               for(int n=0;n<100;n++) nop();
            //    for(int rep=0;rep<100;rep++){
                for(int step = 0; step < train_step-1; step++){
                        // sw prefetch
                        // mprefetch(array2 + (step * stride));
                        if(TEST_ON_SW){
                            mprefetch(array2 + (step * stride));
                            mfence();
                        }else{
                            maccess(array2 + (step * stride));
                            mfence();
                        }
                       
                    }
                if(TEST_ON_SW){
                    mprefetch(array2 + ((train_step - 1) * stride));
                    mfence();
                }else{
                    maccess(array2 + ((train_step - 1) * stride));
                    mfence();
                }
              
              uint64_t dummy = 0;
              for(int k =0;k<100;k++){
                dummy += array1[k*64];
                mfence();
              }

              for(int i=0;i<100;i++) nop(); mfence();

            //   probe_addr = array2 + ((train_step+15) * stride1);

              time1 = timestamp1();
              maccess(array2 + ((train_step+15) * stride1));
              /* READ TIMER */
              time2 = timestamp1();
              res1[atkRound] = (time2-time1);

            //   probe_addr = array2 + ((train_step+15) * stride2);
              time1 = timestamp1();
              maccess(array2 + ((train_step+15) * stride2));
              /* READ TIMER */
              time2 = timestamp1();
              res2[atkRound] = (time2-time1);

            //   printf("Round %d, secret %d, stride %d latency1 %lld, latency2 %lld\n", atkRound, secret[atkRound], stride/64, res1[atkRound], res2[atkRound]);
          } 
      uint64_t attack_end = timestamp1();
        //   printf("%lld\n", latency/rounds);
    
    int correct = 0;
    for(int i=0;i<rounds;i++){
        int guess = res1[i] < res2[i] ? 1 : 0;
        // printf("Round %d, secret %d, stride %d latency1 %lld, latency2 %lld\n", i, secret[i], stride/64, res1[i], res2[i]);
        if(guess == secret[i]) correct++;
        // printf("Round %d, secret %d, stride %d latency1 %lld\n", i, secret[i], stride/64, res1[i]);
    }
    uint64_t elapsed_ns = attack_end - attack_start;
    double elapsed_s = elapsed_ns / 1000000000.0;
    double data_rate_bytes_s = rounds / elapsed_s / 8.0;
    printf("Accuracy: %d/%d\tElapsed_ns: %llu\tElapsed_s: %.9f\tData_rate_Bytes_s: %.3f\n",
           correct,
           rounds,
           (unsigned long long)elapsed_ns,
           elapsed_s,
           data_rate_bytes_s);
    //   }
  return 0;
}
