...
#ifndef VA_MAP_ADDR
#define VA_MAP_ADDR 0x700000000ULL
#endif
...
static void *map_private_page_at(uintptr_t requested_addr) {
    void *mapping = mmap((void *)requested_addr, PAGE_SIZE,
    ...
    if (mapping == MAP_FAILED) {
        die("mmap private page");
    }
    if ((uintptr_t)mapping != requested_addr) {
...
static int child_main(int argc, char **argv) {
    ...
    if (argc != 4) {
        fprintf(stderr, "child usage: %s --child START_FD DONE_FD\n",
                argv[0]);
        return 1;
    }

    start_fd = parse_int_arg(argv[2], "start fd");
    done_fd = parse_int_arg(argv[3], "done fd");

    set_cpu_if_requested();

    shared_page = map_private_page_at((uintptr_t)VA_MAP_ADDR);
...
static void print_header(int stride_bytes, int trigger_line,
    ...
    printf("# arm64 same-VA cross-process %s-stride retention test\n",
#if TRAIN_ACCESS_LOAD
           "load"
#else
           "store"
    ...
    printf("# mapping=private_anonymous fixed_va=0x%016lx same_pa=not_required\n",
           (unsigned long)(uintptr_t)VA_MAP_ADDR);
...
int main(int argc, char **argv) {
    ...
    if (child == 0) {
        ...
        execl(argv[0], argv[0], "--child", start_fd, done_fd, (char *)NULL);
        die("execl child");
    }
