...
SRC = os.path.join(BASE_DIR, "test-cross-process.c")
UTIL_SRC = os.path.join(ROOT_DIR, "until.c")
OUT = os.path.join(ROOT_DIR, "bin", "test-cross-process")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run cross-process store-stride trigger test and report predicted-line prefetching."
    )
    ...
    parser.add_argument("--context-switch", action="store_true",
                        help="Call sched_yield between training and trigger request.")
    parser.add_argument("--parent-trigger", action="store_true",
                        help="Use parent-process trigger as a same-binary baseline. Default: child-process trigger.")
...
args = parse_args()
result_dir = os.path.join(ROOT_DIR, "res", "cross-process")
...
def micro_arch_name(arch, core, train_step):
    ...
    process_suffix = "parent-trigger" if args.parent_trigger else "child-trigger"
    dummy_suffix = ""
    timer = timer_for_arch(arch)
    timer_suffix = ""
    if timer != "gettime":
    ...
    return (
        ...
        f"{timer_suffix}{trigger_suffix}{switch_suffix}-{process_suffix}"
        f"{dummy_suffix}"
    )
...
def run_binary(core):
    cmd = ["taskset", "-c", str(core), OUT]
    if args.parent_trigger:
        cmd.append("--parent-trigger")
...
def run_one(arch, core, train_step):
    ...
    print(
        ...
        f"trigger_process={'parent' if args.parent_trigger else 'child'}"
    )
