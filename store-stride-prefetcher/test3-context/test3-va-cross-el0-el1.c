...
static void print_header(int stride_bytes, int trigger_line,
                         int predicted_line) {
    printf("# arm64 same-VA EL0/EL1 %s-stride retention test\n",
#if TRAIN_ACCESS_LOAD
           "load"
#else
           "store"
    ...
    printf("# EL0 trains %d %ss, %s triggers access %d\n",
           ...
           ,
           TRAIN_ACCESSES);
    printf("# user_page=0x%016lx same_va=yes\n",
           (unsigned long)(uintptr_t)user_page);
    printf("# mapping=private_anonymous fixed_va=0x%016lx\n",
...
int main(void) {
    ...
    user_page = map_private_page_at((uintptr_t)VA_MAP_ADDR);
