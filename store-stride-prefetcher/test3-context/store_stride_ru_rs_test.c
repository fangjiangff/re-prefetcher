...
/* Total train+trigger accesses, matching test3-cross-* semantics. */
#ifndef TRAIN_ACCESSES
#define TRAIN_ACCESSES 6
#endif

#if STRIDE_LINES < 1
#error "STRIDE_LINES must be >= 1"
#endif

#if TRAIN_ACCESSES < 1
#error "TRAIN_ACCESSES must be >= 1"
#endif

#define TARGET_INDEX (TRAIN_ACCESSES * STRIDE_LINES)
#define MAX_INDEX (TARGET_INDEX + 2 * STRIDE_LINES)
...
enum test_mode {
	MODE_BASELINE = 0,
	MODE_STORE_STRIDE,
	MODE_STORE_STRIDE_STORE_TARGET,
	MODE_LOAD_STRIDE,
	MODE_BROKEN_STRIDE,
	MODE_POSITIVE,
	MODE_COUNT
...
static const char *mode_names[MODE_COUNT] = {
	"baseline",
	"store_stride",
	"store_stride_store_target",
	"load_stride",
	"broken_stride",
	"positive_store_T",
};

static const int core1_probe_idx[CORE1_PROBE_N] = {
	TARGET_INDEX,
	TARGET_INDEX + STRIDE_LINES,
	TARGET_INDEX + 2 * STRIDE_LINES,
};
...
static void run_store_stride(struct shared_state *s, uint64_t iter)
{
	...
	for (i = 0; i < TRAIN_ACCESSES; i++)
		mStore_noinline((void *)addr_at(s, i * STRIDE_LINES));
...
static void run_load_stride(struct shared_state *s)
{
	...
	for (i = 0; i < TRAIN_ACCESSES; i++)
		mLoad_noinline((void *)addr_at(s, i * STRIDE_LINES));
...
static void run_broken_stride(struct shared_state *s, uint64_t iter)
{
	...
	for (i = 0; i < TRAIN_ACCESSES; i++) {
		int step = (i & 1) ? TRAIN_ACCESSES - 1 - i / 2 : i / 2;

		mStore_noinline((void *)addr_at(s, step * STRIDE_LINES));
	}

	s->sink1.v += iter;
}

static void run_store_stride_store_target(struct shared_state *s, uint64_t iter)
{
...
static void *prefetcher_thread(void *arg)
{
	...
	for (mode = 0; mode < MODE_COUNT; mode++) {
		for (i = 0; i < total; i++) {
			...
			dummy_accesses(s);
			/* Reset immediately before this round's test accesses. */
			cpp_rctx();
			...
			case MODE_STORE_STRIDE:
				...
				break;
			case MODE_STORE_STRIDE_STORE_TARGET:
				run_store_stride_store_target(s, i);
				break;
...
static void print_stats(struct shared_state *s)
{
	...
	printf("# core0=%d core1=%d iterations=%" PRIu64
	       ...
	       " elem_stride_bytes=%zu stride_lines=%d accesses=%d"
	       " target_index=%d\n",
	       s->cfg.core0, s->cfg.core1, s->cfg.iterations, s->cfg.warmup,
	       s->cfg.delay_nops, s->cfg.elem_stride_bytes, STRIDE_LINES,
	       TRAIN_ACCESSES, TARGET_INDEX);
	printf("# train_sequence=");
	for (access = 0; access < TRAIN_ACCESSES; access++)
		printf("%s%d", access ? "," : "", access * STRIDE_LINES);
	printf(" predicted_index=%d\n", TARGET_INDEX);
...
static void run_core1_probe_case(struct shared_state *s, int explicit_store_target)
{
	uint64_t total = s->cfg.warmup + s->cfg.iterations;
	...
	for (p = 0; p < CORE1_PROBE_N; p++) {
		...
		for (i = 0; i < total; i++) {
			...
			dummy_accesses(s);
			flush_test_array(s);
			/* Match the per-round reset used by test0-exist.c. */
			cpp_rctx();
			run_store_stride(s, i);
			...
			t0 = timestamp();
			mStore_inline((void *)addr_at(s, core1_probe_idx[p]));
			dt = timestamp() - t0;
...
static void usage(const char *prog)
{
	fprintf(stderr,
		...
		"  -s BYTES  bytes per cache-line index, default 64\n"
		"  -h        show this help\n"
		"\n"
		"Example:\n"
...
int main(int argc, char **argv)
{
	...
	s->cfg.elem_stride_bytes = CACHELINE;
