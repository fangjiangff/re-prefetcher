...
OUT = os.path.join(ROOT_DIR, "bin", "test-cross-thread")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run cross-thread store-stride trigger test and report predicted-line prefetching."
    )
    ...
    parser.add_argument("--parent-trigger", action="store_true",
                        help="Use parent-thread trigger as a same-binary baseline. Default: child-thread trigger.")
...
args = parse_args()
result_dir = os.path.join(ROOT_DIR, "res", "cross-thread")
...
def micro_arch_name(arch, core, train_step):
    ...
    thread_suffix = "parent-trigger" if args.parent_trigger else "child-trigger"
    dummy_suffix = ""
    timer = timer_for_arch(arch)
    timer_suffix = ""
    if timer != "gettime":
    ...
    return (
        ...
        f"{timer_suffix}{trigger_suffix}{switch_suffix}-{thread_suffix}"
        f"{dummy_suffix}"
    )
...
def run_one(arch, core, train_step):
    ...
    print(
        ...
        f"trigger_thread={'parent' if args.parent_trigger else 'child'}"
    )
