...
#ifndef TRAIN_STORE_PC4
#define TRAIN_STORE_PC4 0x53e91b4d67c0ull
#endif

#define TRAIN_STORE_GADGETS 5
...
static int init_train_store_gadgets(void) {
    ...
    printf("# train_store_pc4=0x%016lx\n", (unsigned long)pcs[4]);
  ...
  uint64_t rounds = ROUNDS;
    ...
    int latency_sum2 = 0;
          ...
          for(uint64_t atkRound = 0; atkRound < rounds; ++atkRound) {
            ...
            // for(int i=0;i<1000;i++){nop();}
            train_stores[4](array2 + 4 * stride);
            //  for(int i=0;i<1000;i++){nop();}
            // train_stores[1](array2 + 5 * stride);
            //  for(int i=0;i<1000;i++){nop();}
            // train_stores[0](array2 + 6 * stride);
