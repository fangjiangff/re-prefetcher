...
def plot_bar_chart(rows, arch, core):
    ...
    max_position = max(positions) if positions else args.probe_positions - 1
    for boundary in range(CACHE_LINES_PER_4K_PAGE, max_position + 1, CACHE_LINES_PER_4K_PAGE):
        ax.axvline(boundary - 0.5, color="#6A3D9A", linestyle="--", linewidth=1.5)
        ax.text(
            boundary - 0.5, y_limit * 0.99, "4KB page boundary",
            ha="right", va="top", rotation=90, fontsize=12, color="#6A3D9A",
        )
    for page_start in range(0, max_position + 1, CACHE_LINES_PER_4K_PAGE):
        page_end = min(page_start + CACHE_LINES_PER_4K_PAGE - 1, max_position)
        ax.text(
            (page_start + page_end) / 2, y_limit * 0.99,
            f"4KB page {page_start // CACHE_LINES_PER_4K_PAGE}",
            ha="center", va="top", fontsize=12, color="#6A3D9A",
        )
    access_labels = {}
    ...
    max_position = max(positions) if positions else args.probe_positions - 1
    ax.set_xticks(range(0, max_position + 1, 5))
