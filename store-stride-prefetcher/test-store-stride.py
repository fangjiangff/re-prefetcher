...
def parse_args():
    ...
    parser.add_argument("--arch", default=DEFAULT_ARCH,
                        help=f"Architecture label for output files. Default: {DEFAULT_ARCH}")
...
plot_path = os.path.join(plot_dir, f"{micro_arch}-avg_ns.png")
...
def run_tests():
    print("=" * 60)
    print(
        f"access=store, arch={args.arch}, core={args.core}, "
    ...
    raw_path = os.path.join(raw_dir, f"{micro_arch}-store.txt")
...
def plot_bar_chart(rows):
    ...
    ax.set_title("Store instruction (strb)", loc="left", pad=4)
    ...
    fig.suptitle(
        ...
        f"train_step={args.train_step}, hit_threshold={args.hit_threshold_ns} ns",
        x=0.01,
        ha="left",
    )
