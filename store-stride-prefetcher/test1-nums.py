...
SRC = os.path.join(BASE_DIR, "test1-nums.c")
UTIL_SRC = os.path.join(BASE_DIR, "until.c")
OUT = os.path.join(BASE_DIR, "bin", "test1-nums")
...
args = parse_args()
result_dir = os.path.join(BASE_DIR, "res", "store-stride-nums")
...
def run_one(arch, core, train_step, other_pages):
    ...
    if compile_test(train_step, other_pages, arch) != 0:
        print("Compile failed")
        return []
    write_dump(arch, core, train_step, other_pages)
    ...
    raw_path = raw_path_for(arch, core, train_step, other_pages)
    ...
    write_tsv(rows, tsv_path_for(arch, core, train_step, other_pages))
    return rows
...
def main():
    ...
    for arch, core in targets():
        for train_step in train_steps():
            line_points = []
            for other_pages in other_page_counts():
                if args.plot_only:
                    ...
                    rows = run_one(arch, core, train_step, other_pages)
                if rows:
