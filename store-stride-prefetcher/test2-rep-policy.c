...
#define DEFAULT_FILL_COMPETITORS 17
#define DEFAULT_EXTRA_COMPETITORS 1
...
static void trigger_victim(access_gadget_f access_gadget, int stride) {
...
static uint64_t run_one_round(access_gadget_f access_gadget,
                              int fill_competitors,
                              int extra_competitors,
                              int refresh_victim,
                              int page_step_pages,
                              int do_trigger,
                              int stride,
    ...
    flush_victim_lines();
    flush_competitor_lines(fill_competitors + extra_competitors,
                           page_step_pages,
                           stride);
    ...
    // flush_victim_lines();
    if (do_trigger) {
        trigger_victim(access_gadget, stride);
    }
...
static void run_case_probe_map(access_gadget_f access_gadget,
...
static uint64_t run_predicted_case(access_gadget_f access_gadget,
                                   int fill_competitors,
                                   int extra_competitors,
                                   int refresh_victim,
                                   int page_step_pages,
    ...
    for (int r = 0; r < rounds; r++) {
        latency_sum += run_one_round(access_gadget,
                                     fill_competitors,
                                     extra_competitors,
                                     refresh_victim,
                                     page_step_pages,
    ...
    return latency_sum / (uint64_t)rounds;
}

static void print_case(access_gadget_f access_gadget,
                       const char *case_name,
                       int fill_competitors,
                       int extra_competitors,
                       int refresh_victim,
                       ...
                       int predicted_line,
                       int rounds) {
    uint64_t avg = run_predicted_case(access_gadget,
                                      fill_competitors,
                                      extra_competitors,
                                      refresh_victim,
                                      page_step_pages,
...
static void print_usage(const char *prog) {
    fprintf(stderr,
            "usage: %s [access_pc victim_buffer_addr fill_competitors extra_competitors rounds [page_step_pages]]\n"
            "default: access_pc=0x%lx victim_buffer_addr=0x%lx "
            "fill_competitors=%d extra_competitors=%d rounds=%d page_step_pages=%d\n",
            prog,
            (unsigned long)DEFAULT_ACCESS_PC,
            (unsigned long)DEFAULT_VICTIM_BUFFER_ADDR,
            DEFAULT_FILL_COMPETITORS,
            DEFAULT_EXTRA_COMPETITORS,
            DEFAULT_ROUNDS,
...
int main(int argc, char **argv) {
    ...
    int fill_competitors = DEFAULT_FILL_COMPETITORS;
    int extra_competitors = DEFAULT_EXTRA_COMPETITORS;
    ...
    if (argc >= 6) {
        ...
        fill_competitors = atoi(argv[3]);
        extra_competitors = atoi(argv[4]);
        rounds = atoi(argv[5]);
    }

    if (argc == 7) {
        page_step_pages = atoi(argv[6]);
    }

    if (fill_competitors < 0 ||
        extra_competitors < 0 ||
        fill_competitors + extra_competitors > 4096 ||
        rounds <= 0 ||
        page_step_pages <= 0) {
        print_usage(argv[0]);
        return 1;
    ...
    int total_competitors = fill_competitors + extra_competitors;

    if (total_competitors > 0) {
        competitor_span_pages =
            ((size_t)(total_competitors - 1) *
    ...
    printf("# %s-stride replacement-policy FIFO test\n", access_name());
    ...
    printf("# STRIDE_LINES=%d TRAIN_ACCESSES=%d TRIGGER_ACCESSES=%d COMPETITOR_ACCESSES=%d PROBE_POSITIONS=%d fill_competitors=%d extra_competitors=%d page_step_pages=%d\n",
           STRIDE_LINES,
           TRAIN_ACCESSES,
           TRIGGER_ACCESSES,
           COMPETITOR_ACCESSES,
    ...
    printf("# filled:   victim fill competitors trigger probe, without extra competitors\n");
    printf("# baseline: victim fill competitors extra competitors trigger probe\n");
    printf("# refresh:  victim fill competitors refresh-victim extra competitors trigger probe\n");
    printf("case\tfill_competitors\textra_competitors\trefresh_victim\tprobe_line\toffset_bytes\trole\tavg_ns\tprobes\n");

    run_case_probe_map(access_gadget,
                       "no_trigger",
                       0,
                       0,
                       0,
