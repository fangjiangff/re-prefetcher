...
static int find_same_12_33_pair(int *index1, int *index2) {
    ...
    for (int i = 0; i < page_count; i++) {
        ...
        int trigger_candidate = -1;

        for (int j = i; j < page_count && same_key(pages[j].pa) == key; j++) {
            if (scan_range_fits(pages[j].va)) {
                trigger_candidate = j;
                break;
            }
        }
        if (trigger_candidate < 0) {
            continue;
        }
        for (int j = i; j < page_count && same_key(pages[j].pa) == key; j++) {
            if (j != trigger_candidate &&
                pages[j].pa != pages[trigger_candidate].pa &&
                scan_range_fits(pages[j].va)) {
                *index1 = j;
                *index2 = trigger_candidate;
                return 1;
            }
        }
    }
...
int main(void) {
    ...
    printf("# pair_detail\tfound\tva1=0x%016" PRIx64 "\tpa1=0x%016" PRIx64
           ...
           "\tpa_xor=0x%016" PRIx64 "\tsame_key=0x%016" PRIx64 "\n",
