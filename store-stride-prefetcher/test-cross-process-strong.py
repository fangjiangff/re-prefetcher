...
SRC = os.path.join(BASE_DIR, "test-cross-process-strong.c")
UTIL_SRC = os.path.join(BASE_DIR, "until.c")
OUT = os.path.join(BASE_DIR, "bin", "test-cross-process-strong")
...
def parse_args():
    ...
    parser.add_argument("--train-accesses", type=int,
                        ...
                        help="Stores before the exec child trigger. Default: 5")
...
args = parse_args()
result_dir = os.path.join(BASE_DIR, "res", "cross-process-strong")
...
def plot_bar_chart(rows, no_trigger_avg_ns=None, title=None, path=None):
    ...
    if title is None:
        title = f"Strong cross-process {args.access} trigger"
    ...
    legend_items = [
        ...
        Patch(facecolor=ROLE_COLORS["trigger"], edgecolor="black",
              label=f"exec child {args.access} trigger"),
    ...
    if no_trigger_avg_ns is not None:
        no_trigger_text = (
            f", no_trigger_line{predicted_line()}"
